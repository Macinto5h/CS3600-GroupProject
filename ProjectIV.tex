...
\begin{document}
  ...
  \begin{enumerate}[label=\roman*]
    ...
    $FD = \{Artist, Name\} \rightarrow \{Id, Price, Stock, ReleaseDate\}$
  	\item 2NF - The Product table is only 2NF because even though there is the Primary Key Id, you would still be able to get all the same information if you just had the set of \{Artist, Name\}. When you're given those as a candidate key you could easily get \{Id, Price, Stock, ReleaseDate\} from it. This prevents the table from being 3NF, and therefore, BCNF.
  	\item
  	\item
  \end{enumerate}
